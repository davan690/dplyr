...
It's rare that any data analysis undertaken involves using only a single table of data. In practice, you'll normally have many tables that contribute to an analysis, and you need the tools to flexiblily combine them to construct informative interprations. In the R package `dplyr`, there are three families of verbs that allow researchers to work with two tables at a time;
...
All two-table verbs in `dplyr :: _join(...)` work similarly. The first two arguments are `x` and `y`, and provide the tables to combine. The output is always a new table with the same type as `x`.

## Mutating joins

Mutating joins allow you to combine variables from multiple tables. For example, take the nycflights13 data. In one table we have flight information with an abbreviation for carrier (`flights`), and in another we have a mapping between abbreviations and full names (`airlines`). 
...
The explainations above are all found in the `dplyr` package and only contain functions for working with two tables. However within the tidyverse enviroment there are other options for managing more than two tables and other more complex variations for data wrangling. For more than two tables use `purrr::reduce()` or `Reduce()`, as described in [Advanced R](http://adv-r.had.co.nz/Functionals.html#functionals-fp), to iteratively combine the two-table verbs to handle as many tables as you need.
